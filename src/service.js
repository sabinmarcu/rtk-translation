...
import memoize from "rtk-helpers/lib/memoize";
import StateService from "rtk-state/lib/service";
...
export default class TranslatorService {
    ...
    set language(lng: string) {
        if (this.languages.indexOf(lng) >= 0) {
            ...
            console.log("CHANGE TRIGGERED", this._language, this._set, TranslatorService.validSets);
            ::StateService.publish("language.code", { "language.code": this._language });
            ::StateService.publish("language.set", { "language.set": this._set });
        }
    }
